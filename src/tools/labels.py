...
def handle_list_labels(args: dict[str, Any], client: GmailClient) -> dict[str, Any]:
    ...
    for label in labels:
        lines.append(f"  - {label['name']} (id: {label['id']}, type: {label.get('type', 'unknown')})")
...
def handle_modify_thread_labels(args: dict[str, Any], client: GmailClient) -> dict[str, Any]:
    ...
    result = client.modify_thread_labels(
        thread_id=thread_id, add_label_ids=args.get("addLabelIds"), remove_label_ids=args.get("removeLabelIds"),
    )
